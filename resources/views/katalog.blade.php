...
{{-- <div class="flex mx-auto md:block max-w-7xl py-2 sm:px-6 lg:px-8 gap-10 bg-red-100 flex-row">
    <div class="flex flex-row  flex-wrap w-9/12">
        <div class="bg-blue-200 max-h-48 h-full max-w-52  justify-center items-center flex-col flex">
            <img src="img/Minna-no-nihongo-II-scaled.jpg" class="w-full min-w-20 max-w-56" style="" alt="" srcset="">
            <a href="/katalog/posts/{{$post->slug}}" class=" text-blue-500 hover:underline cursor-pointer text-sm">{{$post->title}}</a>
            <div class="flex flex-row  w-full">
                <div class="w-8/12 text-sm flex justify-end">{{$post["harga"]}}</div>
                <a href="/users/{{$post->user->id}}" class="text-xs w-6/12 justify-end items-center flex">{{$post->user->name }}</a>
            </div>
        </div>
...
<section class=" py-8 antialiased dark:bg-gray-900 md:py-12">
        ...
        <div class="flex items-center space-x-4">
          <div class="relative mt-4">
            <form action="/katalog" method="GET">
              ...
              @endif
              <label for="name" class="hidden">Search</label>
              <input type="text" name="search" autocomplete="off" placeholder="search" alt="search" class="rounded-xl max-w-52" id="">
              <div class="absolute inset-y-0 pr-3 pb-3  end-0 flex items-center ps-3 pointer-events-none">
    ...
    <div class="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-6">
          @forelse ($posts as $post)
        <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div class="h-56 w-full">
            <a href="/katalog/posts/{{$post->slug}}">
              <img class="mx-auto h-full dark:hidden" src="https://jpbooks.co.id/files/upload/a98bd3c4cb09ae2b35b0153fa515133eb25dce9c9c7ea96cd2.jpeg" alt="" />
          ...
          <div class="pt-6">
            ...
            <div class="flex justify-between">
              <a href="/katalog/posts/{{$post->slug}}" class="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{{Str::limit($post->title, 7);}}</a>
              <div class="  inline-flex items-center rounded-lg bg-{{$post->kategori->color}}-100 px-3 py-1 text-sm font-medium  text-grey-700">
                <a href="/katalog?kategori={{$post->kategori->slug}}" class="text-md  leading-tight hover:underline dark:text-white">{{$post->kategori->name}}</a> 
              </div>
            </div>
            <div class="mt-4 flex items-center justify-between gap-4">
              <p class="text-2xl font-extrabold leading-tight text-gray-900 dark:text-white">{{$post->harga}}</p>
            </div>
          </div>
        </div>
        ...
        @empty 
        <div class="h-96 ">
          <p class="font-semibold text-xl overflow-hidden w-11/12 flex justify-center">Product Not Found</p>
        </div>
...
{{-- mobile display --}}
<div class="md:hidden mx-auto  max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
